    ...
    'anthropic': {
        ...
        'models': {
            'claude-sonnet-4-20250514': {
                ...
                'description': 'Latest and most intelligent Claude',
                'input_cost': 3.00,
                'output_cost': 15.00,
                'icon': '🌟',
                'speed': 'Fast',
...
async def initialize_provider(provider_name, model, temperature, max_tokens):
    ...
    try:
        ...
        st.error(f"{provider_name.title()} initialization failed: {str(e)}")
...
if not st.session_state.initialized:
    ...
    # Set default provider based on settings.default_provider
    default_prov = settings.default_provider.lower() if hasattr(settings, 'default_provider') else None
    
    if default_prov and default_prov in available_providers:
        st.session_state.current_provider = default_prov
        # Get default model from settings
        if default_prov == 'openai':
...
with st.sidebar:
    ...
    if st.session_state.current_provider:
        ...
        for model_id, config in available_models.items():
            with st.expander(
                ...
                st.caption(f"💰 ${config['input_cost']:.2f}/1M in • ${config['output_cost']:.2f}/1M out")
    ...
    with st.container():
        ...
        if temperature < 0.3:
            st.info("❄️ Very Focused - Deterministic")
        elif temperature < 0.7:
            st.info("🎯 Balanced - Recommended")
        elif temperature < 1.3:
            st.info("🎨 Creative - Varied")
        else:
            st.info("🌈 Very Creative - Experimental")
        
        st.divider()
        
        # Max Tokens
        st.markdown("**📊 Max Output Tokens**")
        st.caption("Maximum response length")
    ...
    if st.session_state.current_provider and st.session_state.current_model:
        ...
        <div class="model-card">
            ...
            <p><strong>Model:</strong> {st.session_state.current_model}</p>
    ...
    # Cost status
    if tracker.hard_limit and tracker.hard_limit > 0:
    ...
    with col1:
        if st.button("💾 JSON", use_container_width=True):
            if st.session_state.conversation.count_messages() > 0:
                ...
                filepath = f"exports/chat_{timestamp}.json"
    ...
    with col2:
        if st.button("📄 MD", use_container_width=True):
            if st.session_state.conversation.count_messages() > 0:
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                filepath = f"exports/chat_{timestamp}.md"
                os.makedirs("exports", exist_ok=True)
...
with messages_container:
    if st.session_state.conversation.count_messages() == 0:
        ...
        <div class="model-card" style="text-align: center; padding: 2rem;">
            ...
            <p>This is a production-grade multi-provider AI assistant.</p>
            <p>💡 <strong>Features:</strong></p>
            <ul style="text-align: left; display: inline-block;">
                <li>Multi-provider support (OpenAI & Anthropic)</li>
                <li>Real-time cost tracking</li>
                <li>Conversation export (JSON/Markdown)</li>
                <li>Advanced configuration options</li>
            </ul>
            <p>🚀 Configure your settings in the sidebar and start chatting!</p>
        </div>
        """, unsafe_allow_html=True)
    else:
...
async def process_message(user_input):
    ...
    try:
        ...
        logger.info(f"Response generated successfully. Cost: ${cost:.6f}")
...
<div style="text-align: center; color: #7f8c8d; font-size: 0.9rem;">
    <strong>AI Assistant Pro</strong> | 
    Built for the NexusBerry Full-Stack AI Engineer Course | 
    <a href="https://github.com/TalhaRana01/AI-Assistant-with-Anthropic-and-OpenAI-Model" target="_blank">GitHub Repository</a>
    <br>
    💡 <strong>Tip:</strong> Lower temperature (0-0.5) for factual tasks, higher (1-2) for creative work
</div>
<<<<<<< HEAD
""", unsafe_allow_html=True)
=======
